...
#include <string_view>
#include <vector>
bool supported_interpreter(){
#ifdef Py_GIL_DISABLED
  PyErr_SetString(PyExc_ImportError,"native bindings require the GIL");return false;
#else
  if(PyInterpreterState_GetID(PyInterpreterState_Get())!=0){PyErr_SetString(PyExc_ImportError,"native bindings require the main interpreter");return false;}
  return true;
#endif
}
...
struct RecursionGuard {
  ...
  RecursionGuard(){if(depth>=128){PyErr_SetString(PyExc_RecursionError,"message nesting exceeds 128 levels");throw std::runtime_error("message nesting limit");}++depth;}
  ~RecursionGuard(){--depth;}
};
thread_local unsigned RecursionGuard::depth=0;
struct DictSnapshot {
...
struct Owner {
  ...
  Owner(PyObject* data) { auto* p=PyBytes_AS_STRING(data);auto n=PyBytes_GET_SIZE(data);if(PyBytes_CheckExact(data)&&reinterpret_cast<uintptr_t>(p)%8==0){reader=std::make_unique<capnp::FlatArrayMessageReader>(kj::arrayPtr(reinterpret_cast<const capnp::word*>(p),n/8));pinned=Py_NewRef(data);}else{words=kj::heapArray<capnp::word>(n/8);memcpy(words.begin(),p,n);reader=std::make_unique<capnp::FlatArrayMessageReader>(words.asPtr());} }
  ~Owner(){reader.reset();Py_XDECREF(pinned);}
  Owner():builder(std::make_unique<capnp::MallocMessageBuilder>()){}
};
struct Handle {
  std::shared_ptr<Owner> owner;capnp::_::StructReader reader;int schema;bool is_root=false;
  ...
  Handle(std::shared_ptr<Owner> o,capnp::_::StructBuilder b,int s,bool root=false):owner(std::move(o)),reader(b.asReader()),schema(s),is_root(root),builder(std::make_unique<capnp::_::StructBuilder>(b)){}
};
struct Slot {alignas(Handle) unsigned char bytes[sizeof(Handle)];};
Handle* construct(Slot* s,Handle* h) noexcept {try{return new(s->bytes) Handle(std::move(*h));}catch(const std::bad_alloc&){PyErr_NoMemory();return nullptr;}}
...
uint16_t enumerant(PyObject* v,std::initializer_list<const char*> names){if(PyLong_Check(v))return integer<uint16_t>(v);if(!PyUnicode_Check(v))throw std::runtime_error("expected enum name or integer");unsigned i=0;for(auto name:names){if(PyUnicode_CompareWithASCIIString(v,name)==0)return i;++i;}throw std::runtime_error("unknown enumerant");}
...
PyObject* interned(const char* name){auto it=union_names.find(name);if(it==union_names.end()){auto* s=PyUnicode_InternFromString(name);if(!s)return nullptr;Owned temporary(s);it=union_names.emplace(name,s).first;temporary.release();}return Py_NewRef(it->second);}
